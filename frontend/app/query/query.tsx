  ...
  function updateQueryTracker(prevQueryTracker: number): number {
    ...
    if (storedTime && now.getTime() > JSON.parse(storedTime).expiration) {
      ...
      localStorage.setItem(
        "nsqueries",
        JSON.stringify({ value: 0, expiration: expirationDate })
      );
      return 0;
    }
    ...
    localStorage.setItem(
      "nsqueries",
      JSON.stringify({ value: updatedQueryTracker, expiration: expirationDate })
    );
    return updatedQueryTracker;
  }
  ...
  useEffect(() => {
    ...
    async function fetcher() {
      try {
        ...
        let collectionParam: string | string[] | null =
          ...
          for (let collection of collectionParam) {
            const cardsReq = await fetch(
              `https://www.nationstates.net/cgi-bin/api.cgi?q=cards+${`collection;collectionid=${collection}`}`,
              {
                headers: {
                  "User-Agent":
                    "Kractero Card Queries but the person running this is probably not Kractero",
                },
              }
            );
            setQueryTracker((prevQueryTracker) =>
              updateQueryTracker(prevQueryTracker)
            );
            const cardsText = await cardsReq.text();
            const parser = new XMLParser();
            let cards = parser.parse(cardsText);
            if (
              !(cards.CARDS.COLLECTION && cards.CARDS.COLLECTION.DECK.CARD) &&
              !(cards.CARDS.DECK && cards.CARDS.DECK.CARD)
            ) {
              throw new Error(
                "Something is wrong with the collection or deck you gave"
              );
            }
            cards.CARDS.COLLECTION.DECK.CARD.forEach(
              (card: { CARDID: string; SEASON: number; CATEGORY: string }) => {
                for (let obj of collectionCards) {
                  if (obj.CARDID === card.CARDID && obj.SEASON === card.SEASON)
                    return;
                }
        ...
        if (collectionCards.length > 0 && deckCards.length > 0) {
          ...
          const getCards = await fetch(
            `${process.env.NEXT_PUBLIC_API}/collection?${lastQuery}`,
            {
          ...
          const getCards = await fetch(
            `${process.env.NEXT_PUBLIC_API}/cards?${lastQuery}`,
            {
              signal: abortController.signal,
            }
          );
    ...
    <MantineProvider emotionCache={appendCache}>
      <div className="flex gap-4">
        <a href="/">
          <Button
            variant={"outline"}
            data-umami-event="Make New Query"
      ...
      <small className="text-xs leading-none mb-4">
        (50/30 seconds allowed)
      </small>
      {correspondingJson.length > 0 ? (
            ...
            <div className="flex gap-4 mt-4">
              <DropdownMenu>
                ...
                <DropdownMenuContent>
                  ...
                  <DropdownMenuItem onSelect={() => setSortValue("ID")}>
                    Card ID
                  </DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => setSortValue("Rarity")}>
                    Rarity
                  </DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => setSortValue("Name")}>
                    Name
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
        ...
        <p className="dark:text-white text-lg font-bold mb-2">
          Generating cards for {lastQuery}, please wait...
        </p>
      )}
